...
: t> ( ? -- n )
  23 . B0 . 59 . 00 . ;		                            \ [s3] = t0                             ld t0, 0(s3)

: !>t ( n -- )  >t  ^ ;
: t>! ( -- n )  v  t> ;

: :: ( ub1 ub2 ub3 ub4 -- )
  [ ^ ^ ^ >t v v v t>! ] .                                  \ [s1++] = ub1  ( -- ub1 ub2 ub3 ub4 )
  [ ^ ^   >t   v v t>! ] .                                  \ [s1++] = ub2  ( -- ub1 ub2 ub3 ub4 )
  [ ^     >t     v t>! ] .                                  \ [s1++] = ub3  ( -- ub1 ub2 ub3 ub4 )
...
\ : LIT ( C: u -- ) ( -- u )
\   \ lui t0, 0x?????  ->  ?????2B7
\   [ 0009E283 $ ]		                            \ u@t0 = [s3]                           lwu t0, 0(s3)
\   [ 00001337 $ ]					    \ mask@t1 = 0x00000FFF                  lui t1, 1
\   [ FFF30313 $ ]					    \                                       addi t1, t1, -1
\   [ 0062F333 $ ]					    \ low@t1 = u@t0 & mask@t1               and t1, t0, t1
\   [ 00B35393 $ ]					    \ sign@t2 = low@t1 >> 11                srli t2, t1, 11
\   [ 00C2D293 $ ]					    \ high@t0 = u@t0 >> 12                  srli t0, t0, 12
\   [ 007282B3 $ ]                                          \ high'@t0 = high@t0 + sign@t2          add t0, t0, t2
\   [ 00C29293 $ ]                                          \ lui@t0 = (high'@t0 << 12) | 0x2B7     slli t0, t0, 12
\   [ 2B72E293 $ ]					    \                                       ori t0, t0, 0x2B7
\   [ 0059B023 $ ] $					    \ [s3] = t0; [s1++4] = "lui ..."        sd t0, 0(s3)           ( C: -- )
\   \ addi t0, t0, 0x???  ->  ???28293
\   [ 282932B7 $ ]                                          \ addi@t0 = (low@t1 << 20) | 0x28293    lui t0, 0x28293
\   [ 00C2D293 $ ]					    \                                       srli t0, t0, 12
\   [ 01431313 $ ]					    \                                       slli t1, t1, 20
\   [ 0062E2B3 $ ]					    \                                       or t0, t0, t1
\   [ FF898993 $ ]                                          \ s3 -= 8                               addi s3, s3, -8
\   [ 0059B023 $ ] $					    \ [s3] = t0; [s1++4] = "addi ..."       sd t0, 0(s3)           ( C: -- )
\   \ addi s3, s3, -8  ->  FF898993
\   [ FF898993 $ ]                                          \ [s1++4] = "addi ..."
\   \ sd t0, 0(s3)  ->  0059B023
\   [ 0059B023 $ ] ;                                        \ [s1++4] = "sd ..."

: ^ ( n -- )
  00898993 $ ;                                              \ s3 += 8                               addi s3, s3, 8
: v ( n -- )
  FF898993 $ ;                                              \ s3 -= 8                               addi s3, s3, -8
...
\ : .>y ( n -- n )
\   83 . BA . 09 . 00 . ;				            \ x@s5 = [s3]                           ld s5, 0(s3)
\ : y>. ( ? -- n )
\   23 . B0 . 59 . 01 . ;	       			            \ [s3] = x@s5                           sd s5, 0(s3)
  
: x<< ( u -- )
  \ slli s4, s4, ?
  \ -> ~000000~​{{{shamt(??.????)}}} {{{rs1(1010.0)}}} {{{fn3(001)}}} {{{rd(1010.0)}}} {{{op(0010011)}}}
  \ -> 13 1A ?A 0?
  13 . 1A .	                                            \ [s1++] = ...

  [ .>t0 ]
  [ 93 . F2 . F2 . 00 . ]                                   \ t0 &= 0b00001111                      andi t0, t0, 0x0F
  [ 93 . 92 . 42 . 00 . ]                                   \ t0 <<= 4)                             slli t0, t0, 4
  [ 93 . E2 . A2 . 00 . ]                                   \ t0 |= 0b00001010                      ori t0, t0, 0x0A
  [ v t0>. ] .

  [ .>t0 ]
  [ 93 . D2 . 22 . 00 . ]				    \ t0 >>= 2                              srli t0, t0, 2
  [ t0>. ] . ;

: x| ( u -- )
  \ ori s4, s4, ?
  \ -> {{{imm(0x???)}}} {{{rs1(10100)}}} {{{fn3(110)}}} {{{rd(10100)}}} {{{op(0010011)}}}
  \ -> 13 6A ?A ??    |     ???A6A13
  [ .>t0 ^ ]                                                \ t0 = u
  [ A6A13 .>t1 ^ ]					    \ t1 = A6A13
  [ 01429293 $ ]	                                    \ t0 <<= 20                             slli t0, t0, 20
  [ 0062E2B3 $ ] $ ;					    \ t0 |= t1                              or t0, t0, t1
...
: 1+ ( n -- n' )
  [ .>x ]
  [ 13 . 0A . 1A . 00 . ]	                            \ x@s4 += 1                             addi s4, s4, 1
  [ x>. ] ;
