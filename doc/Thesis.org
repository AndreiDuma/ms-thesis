...
[cite:@forth_2012_standard] and supports multiple target
architectures.  Being featureful and performant, it is a rather
complex compiler.  Its core is implemented in the C programming
language with many words and libraries being written in Forth itself.
...
with the consequence that its assembly part is longer than what it
could be.
...
system.  It is extremely minimal, providing only several primitives
out of which a more featureful Forth.  /MilliForth/ [cite:@milliforth]
takes sectorforth's minimalism even further, reducing the machine code
size to 340 bytes.
...
to Threaded-Code Systems''/ (1982), which explains the design of
...
We think /SmithForth/ is an excellent starting point for creating a
new Forth implementation that focuses on understandability.  We
propose a number of modifications that aim towards this goal:
