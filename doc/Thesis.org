...
Choosing the right language to implement is important, of course, if
building a compiler for it is to convey useful knowledge (or even
wisdom) to the student.  The language should be simple enough so that
its compiler does not grow beyond comprehension.  Still, it should be
rich enough to expose the student to the implementation of general
...
programming language implementation.  We continue by reviewing
existing Forth implementations.
