...
- /Compiling and executing/ turns executions tokens into latent or
...
character already parsed.  The /parse area/ is defined as the range
...
{{{wtable_begin(10)}}}
#+CAPTION: ~':'~ sets up a word's dictionary entry and begin compilation (x86-64).
...
~HIDDEN~ field in the dictionary entry and switches back to
interpreting mode.
...
and out of interpreting state.  This enables many tricks by allows the
mixing of compilation-time and run-time code execution.  The two
...
in definitions instead of being compiled as a call sequence as
non-immediate words are.

The following word, shown in Table [[tbl:forth:.:x]], is equivalent in
functionality to standard Forth word ~'C,'~​, pops a byte off the data
stack and writes it, literally, to ~OUTPUT~.  This word is used
...
given string (arguments ~addr~ and ~u~).  A few observations are
worthwhile.  First, the test fails if the word is hidden: this is to
prevent colon definitions to reference themselves before they are
complete.  Secondly, being designed for the x86-64, SmithForth uses a
combined ~REPE CMPS~ instruction for the string comparison.  This will
need to be replaced with an explicit loop in our RISC-V
implementation.  Another feature specific to x86-64 is the use of the
implicit ~RFLAGS~ to pass information between instructions (and even
subroutines).  The ~ZF~ flags set here by SmithForth is read only in
~FIND~.
...
code implementation of SmithForth): it uses a ~COMPL~ to compile a
...
different level.  Chain-called one after the other, they receive an
...
machine code.  Here two final words combine words from the five
...
Table [[tbl:forth:ti:x][tbl:forth:ti:x]] showcases ~ti~, the word which, in Forth fashion,
brings everything together.  It first sets up the data stack right
below the global variables.  This very interesting in practice,
because any accidental data stack underflow might corrupt the global
...
*** Searching the Dictionary: ~xt=~, ~FIND~                        :noexport:
...
*** Handling Numbers: ~Num~, ~miss~                                :noexport:
...
*** Compile or Execute: ~EXEC~, ~exec~, ~compl~, ~hit~             :noexport:
...
*** Bringing Everything Together: ~SVAL~, ~ti~                     :noexport:
